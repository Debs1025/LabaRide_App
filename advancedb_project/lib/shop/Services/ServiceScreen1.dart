...
class _ServiceScreen1State extends State<ServiceScreen1> {
  ...
  Future<void> _loadServices() async {
    ...
    try {
      final response = await http.get(
        Uri.parse(
          '${SupabaseConfig.apiUrl}/shop/${widget.shopData['id']}/services',
        ),
        headers: {
  ...
  Future<void> _loadKiloPrices() async {
    try {
      ...
      print(
        'Loading kilo prices... Status: ${response.statusCode}',
      ); // Debug print
            ...
            if (data['prices'] != null) {
              for (var price in data['prices']) {
                ...
                kiloPrices[key] = KiloPrice(
                  ...
                  pricePerKilo: double.parse(
                    price['price_per_kilo'].toString(),
                  ),
                );
              }
            }
  ...
  Future<void> _addService(String name, Color color, double price) async {
    ...
    try {
      final response = await http.post(
        Uri.parse(
          '${SupabaseConfig.apiUrl}/shop/${widget.shopData['id']}/service',
        ),
        headers: {
          'Authorization': 'Bearer ${widget.token}',
          'Content-Type': 'application/json',
  ...
  Future<void> _saveKiloPrice() async {
    try {
      ...
      if (minKilo >= maxKilo) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Maximum kilo must be greater than minimum kilo'),
          ),
        );
        return;
      }
      ...
      final response = await http.post(
        Uri.parse(
          '${SupabaseConfig.apiUrl}/shop/${widget.shopData['id']}/kilo-price',
        ),
        headers: {
          'Authorization': 'Bearer ${widget.token}',
          'Content-Type': 'application/json',
      ...
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('Error saving price range: $e')));
      }
    }
  }
  ...
  Future<void> _deleteKiloPrice(double minKilo, double maxKilo) async {
    try {
      final response = await http.delete(
        ...
        body: jsonEncode({'min_kilo': minKilo, 'max_kilo': maxKilo}),
      );
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('Error deleting price range: $e')));
    }
  }
                    ...
                    DropdownButton<Color>(
                      ...
                      items: const [
                        DropdownMenuItem(
                          ...
                          child: CircleAvatar(
                            backgroundColor: Color(0xFF1A0066),
                          ),
                        ),
                        DropdownMenuItem(
                          value: Color(0xFF5AB090),
                          child: CircleAvatar(
                            backgroundColor: Color(0xFF5AB090),
                          ),
                        ),
                        DropdownMenuItem(
                            ...
                            kiloPrices.isEmpty
                                ...
                                : ListView.builder(
                                  ...
                                  itemBuilder: (context, index) {
                                    final entry = kiloPrices.entries.elementAt(
                                      index,
                                    );
                                                ...
                                                ? Row(
                                                  ...
                                                  children: [
                                                    ...
                                                    IconButton(
                                                      ...
                                                      onPressed:
                                                          () =>
                                                              _showKiloPriceDeleteConfirmation(
                                                                context,
                                                                price.minKilo,
                                                                price.maxKilo,
                                                              ),
  ...
  void _showKiloPriceDeleteConfirmation(
    ...
    showDialog(
      ...
      builder:
          (context) => AlertDialog(
            ...
            content: Text(
              'Are you sure you want to delete the ${minKilo}kg - ${maxKilo}kg range?',
            ),
            actions: [
              TextButton(
                onPressed: () => Navigator.pop(context),
      ...
      body: SingleChildScrollView(
        child: Padding(
          ...
          child:
              _isLoading
                  ? const Center(
                    child: CircularProgressIndicator(color: Color(0xFF1A0066)),
                  )
                  : Column(
